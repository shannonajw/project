...
\setlength{\droptitle}{-2em}
  ...
  \postdate{\par}
    \date{Last updated 22 September 2019}
...
this?} Well, this is a period measure - it records the number of
marriages in a certain year. Unnamed `observers' cited by the article
...
\subsection{Different measures, same
story}\label{different-measures-same-story}
...
\textbf{Step 1} For a start, let us try to do Step 1 and make sure that
changes in period measures are \emph{not} due to the changing population
age structure (i.e.~age effects). Age-specific marriage rates are
adjusted for the size of age groups and give us a picture of period
change that is not driven by changes in age composition. They are thus
very helpful here, and have been provided in
...
combination of age and period data suggest that the presence cohort
change remains plausible (Step 1).

\textbf{Step 2} Step 2 is to verify the cohort explanation itself. NUS
sociologist Tan Ern Ser says that younger cohorts are delaying marriage
in view of education and career building, so we should check that
younger cohorts are indeed spending longer in education/early-career.
The Ministry of Education happens to provide us data on Primary 1
cohorts (i.e., people who attended Primary 1 in the same year are in the
same cohort), which tend to approximate birth cohorts closely (Figure
...
courses overall (see the rising trend across years). More evidence is
needed to confirm whether this (and ``early-career building'') is in
fact the cause of delayed marriages, but it does seem like younger
cohorts are spending more time in education. \emph{The cohort
explanation therefore seems reasonable.}

You may now ask - so perhaps looking at a mix of period-based and
age-based measures can provide us some indication of cohort change? The
answer is that it depends\footnote{It depends on many factors, such as
  how they were calculated, what their interpretations are, whether they
  are population or sample data, and more.}. In this case, it seems like
the age and period measures do signal cohort trends, and the cohort
explanation is viable (though it cannot be confirmed with the data we
have seen). However, there are times when cohort and period trends will
not cohere, as we will see shortly.

\subsection{Different measures, different
story}\label{different-measures-different-story}
...
Regardless of whether think it is a period or cohort explanation, a
natural follow-up question would be - does this also explain the reason
why divorce rates are falling? If the `waiting to be very sure'
explanation holds weight, it should follow that since people have become
more careful to marry, marriages are probably less likely to end up in
divorce\footnote{It could also be that \emph{even though} people are
  more careful, they still make choices that end up in divorce, but this
  is somewhat an awkward assumption to make given a plain reading of the
  comment.}! People ``waiting longer'' also would then explain why
divorce numbers are falling divorces.
...
therefore taking longer) about choosing their partners.}
...
\textbf{period} measure\footnote{You may also ask, since we're dealing
  ...
  size of the group at risk.}. \emph{Also remember the steps outlined in
...
\textbf{Step 1} Once again, we can look at age-specific rates to
determine if the explanation is plausible (Figure
...
Once again we see in Figure \ref{fig:apc-reportsingstat2nd} that in
2018, divorce rates among younger age groups (15-29) are lower and
higher in older age groups (30+), compared to 2008. Divorce numbers thus
do not seem to be driven by changes in population age composition.

This is the same situation as before (with marriage numbers), so perhaps
we can use a mix of period-based and age-based measures to support the
existence of cohort change. But how well do these age and period numbers
cohere with \emph{cohort} trends?

Data on divorce trends by birth cohort are (again) not available from
the government. But all is not lost. Weirdly enough, there exists some
...
to say, but for heuristic purposes we may assume they roughly
...
trends (and the news article). \textbf{While period trends suggested
that divorce rates were falling overall}\footnote{Note that the
  ...
  delayed divorces.}\textbf{, cohort trends seem to be telling the
opposite story - younger cohorts seem to be getting divorced earlier
into their marriages!}

\textbf{Step 2} The cohort explanation we adopted for this section was
that ``Divorce numbers are falling because younger cohorts are more
careful about choosing their partners''. We have already found, however,
younger cohorts (albeit on marriage cohorts, not birth cohorts) are in
fact getting divorced earlier. This directly contradicts the
...
change, there are times when they do not.} When we say ``marriages and
divorces are falling'', we need to be clear what the data allows us to
say, and what kinds of explanations we give. If cohort trends are not
the same as period trends, than providing cohort explanations for period
change becomes meaningless.

In the next section, we will look at life expectancy - a highly
misunderstood measure in popular discourse.
