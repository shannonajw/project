...
``internet use seniors''. One 2014 article in the Straits Times caught
my eye (see Figure \ref{fig:st-websavvyseniors}).
...
the news article was published. Note that the percent of Singaporeans
aged 55 and above who use the internet daily in 2014 \textbf{is 29\%,
not 78\% as the article suggests.}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Set the number of trials to 4. You may change the number to see what happens.}
\NormalTok{n <-}\StringTok{ }\DecValTok{4} 
\CommentTok{# Calculate the proportion of heads, based on flipping a fair coin. Try this a few times.}
...
is \(\Pr(\text{Support Ban})\) - the total probability. Using the law of
