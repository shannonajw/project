...
\setlength{\droptitle}{-2em}
  ...
  \postdate{\par}
    \date{Last updated 25 July 2019}
...
that we can determine what is the `truth' in a hypothetical population,
and then use the computer to draw samples from this population\footnote{The
  code here utilizes the Monte Carlo method, which essentially is a
  computational way to see how samples perform under realistic data
  conditions with a known probability distribution.} to see how sampling
(and sample sizes) work. Our quantity of interest is the proportion of
people who like durian. We define the population such that 65\% of
people (in our population) like durian (i.e.,
...
produced is different each time you draw a new sample. This makes sense,
since samples are drawn randomly. Since we know the true value of
...
fall between 0.56 and 0.74. This gives us some measure of uncertainty.
...
fundamental basis of claims by researchers that a survey has a certain
``margin of error''.
...
from researchers, but this is certainly not a bad thing. Because of the
practical drawbacks of probability sample highlighted above, statistical
...
\emph{all} of the claims in that study are invalid. There is still much
to learn from it. However, it does tell us that we need to be constantly
evaluate the claims of studies against their methodology - understanding
the \emph{how} is often key.
