...
Note here that median age at marriage is also a *period measure* - of all people who got married that year, half of them got married before age 28.5. 
...
The chart above shows that younger cohorts are indeed spending longer in education, since a larger percentage of them end up in post-secondary courses overall (see the rising trend across years). More evidence is needed to confirm whether this (and "early-career building") is in fact the cause of delayed marriages, but it does seem like younger cohorts are spending more time in education. *The cohort explanation therefore seems reasonable.*

You may now ask - so perhaps looking at a mix of period-based and age-based measures can provide us some indication of cohort change? The answer is that it depends^[It depends on many factors, such as how they were calculated, what their interpretations are, whether they are population or sample data, and more.]. In this case, it seems like the age and period measures do signal cohort trends, and the cohort explanation is viable (though it cannot be confirmed with the data we have seen). However, there are times when cohort and period trends will not cohere, as we will see shortly. 

### Different measures, different story
...
Regardless of whether think it is a period or cohort explanation, a natural follow-up question would be - does this also explain the reason why divorce rates are falling? If the 'waiting to be very sure' explanation holds weight, it should follow that since people have become more careful to marry, marriages are probably less likely to end up in divorce^[It could also be that *even though* people are more careful, they still make choices that end up in divorce, but this is somewhat an awkward assumption to make given a plain reading of the comment.]! People "waiting longer" also would then explain why divorce numbers are falling divorces. 

*Suppose we adopt the following explanation*: **Divorce numbers are falling because younger cohorts are more careful (and therefore taking longer) about choosing their partners.**

Remember that the divorce numbers mentioned in the news article are a **period** measure^[You may also ask, since we're dealing with absolute numbers, isn't it obvious that if there are less marriages, then there would be less divorces too? This is why absolute numbers are seldom helpful when examining societal change. At a minimum, we should attempt to look for rates that are adjusted for the size of the group at risk.]. *Also remember the steps outlined in the previous sub-section*:
...
Once again, we can look at age-specific rates to determine if the explanation is plausible (Figure \@ref(fig:apc-reportsingstat2nd)). 
...
Once again we see in Figure \@ref(fig:apc-reportsingstat2nd) that in 2018, divorce rates among younger age groups (15-29) are lower and higher in older age groups (30+), compared to 2008. Divorce numbers thus do not seem to be driven by changes in population age composition. 

This is the same situation as before (with marriage numbers), so perhaps we can use a mix of period-based and age-based measures to support the existence of cohort change. But how well do these age and period numbers cohere with  *cohort* trends?

Data on divorce trends by birth cohort are (again) not available from the government. But all is not lost. Weirdly enough, there exists some information on divorce by "marriage cohorts" (where each cohort is defined by the year they got married - i.e., if you got married in the same year as I did, you are in my marriage cohort) in a report by the Ministry of Social and Family Development (found [here](https://www.msf.gov.sg/research-and-data/Research-and-Data-Series/Pages/default.aspx){target="_blank"}). What kind of change marriage cohort trends really capture is quite hard to say, but for heuristic purposes we may assume they roughly approximate^[Think about who gets married in a certain year. What kind of people are they composed of? What characteristics do they share? It is hard to say. Other than sharing the same year of marriage, diversity within marriage cohorts is likely to be huge. We can only assume that younger birth cohorts will dominate most of the marriages in later years, but it remains to be seen whether this is accurate.] birth cohort trends. Figure \@ref(fig:apc-reportmsf) shows us the proportion of divorces (y-axis) that happen before the couples' $x^{th}$ anniversary (shown on the x-axis), by marriage cohort (indicated by the color and shape of the lines).
...
In the next section, we will look at life expectancy - a highly misunderstood measure in popular discourse.
