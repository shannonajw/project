...
If you run the code above multiple times, you will realize the number produced is different each time you draw a new sample. Since we know the true value of $\Pr(\text{Likes Durian})$, we also know how far off these sample estimates of $\Pr(\text{Likes Durian})$ tend to be. The next step in this exercise is to draw a large number of samples of the same size to show how wide these estimates can vary.
...
You will realize here that the interval between the quantile values have narrowed^[If you calculate the mean of proportions again, you will also realize also that it recovers the true value of 0.65]. This shows that with a bigger sample size, our uncertainty around the true estimate also shrinks. 
...
What does this case study highlight? First, we need to be transparent^[And we need to demand transparency.] about how studies recruit members into their sample. **How the sample is recruited is usually much more important than how large the sample is**, but we are seldom given enough detail about it. Second, we need to be aware if appropriate methods are being used. Using a non-probability sample the more practical option, but we become overconfident and may end up with the wrong conclusions if we simply act as if it is a probability sample.
